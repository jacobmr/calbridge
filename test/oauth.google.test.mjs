...
describe("Google OAuth", () => {
  ...
  it("init redirects to Google with state param (signup when no session)", async () => {
    ...
    assert.equal(
      locUrl.searchParams.get("redirect_uri"),
      "https://mical.net/api/oauth/google/callback",
    );
  ...
  it("init uses link intent when session cookie present", async () => {
    ...
    const req = mockReq({
      url: "/api/oauth/google/init?return_to=/app/",
      cookie,
    });
    const res = mockRes();
  ...
  it("callback creates user, tenant, and oauth_account on first sign-in", async () => {
    ...
    const req = mockReq({
      url: `/api/oauth/google/callback?code=test-code&state=${state}`,
    });
    const res = mockRes();
    ...
    assert.equal(
      ...
      "callback should append ?connected=google so the dashboard can show its post-OAuth toast",
    );
    ...
    const tenantRows = await db.execute({
      sql: "SELECT id, slug, owner_user_id FROM tenants WHERE owner_user_id = ?",
      args: [userRows.rows[0].id],
    });
  ...
  it("callback updates existing account on re-auth", async () => {
    ...
    const acctRows = await db.execute({
      sql: "SELECT COUNT(*) as c FROM oauth_accounts",
    });
