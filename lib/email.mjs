...
/**
 * Build + send a "you've been invited to vote on this poll" email. Sent
 * once per invitee at poll creation time. The link goes to /poll/:token,
 * which works for anyone with the link (the email is just the delivery
 * mechanism — security is the unguessable token).
 */
export async function sendPollInviteEmail({
  ...
  const inviter = organizerName || organizerEmail || "Someone";
  const subject = `${inviter} wants you to pick a time for "${pollTitle}"`;
...
<html><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#1a202c;background:#f5f7fa;margin:0;padding:32px 16px;">
  <div style="max-width:520px;margin:0 auto;background:#ffffff;border-radius:12px;padding:40px 32px;box-shadow:0 2px 12px rgba(0,0,0,0.04);">
    ...
    <h1 style="font-size:1.4rem;font-weight:700;margin:0 0 16px;color:#1a202c;letter-spacing:-0.01em;">
      ${esc(inviter)} needs you to pick a time
    </h1>
    <p style="color:#4a5568;line-height:1.55;margin:0 0 16px;font-size:0.98rem;">
      For: <strong>${esc(pollTitle)}</strong>
    </p>
    <p style="color:#4a5568;line-height:1.55;margin:0 0 24px;font-size:0.98rem;">
      Click below, mark which proposed times work for you, and we'll let ${esc(inviter)} pick the winner. If you sign in with your calendar, each option will be pre-marked free or busy automatically.
    </p>
    <p style="text-align:center;margin:32px 0;">
      <a href="${esc(pollUrl)}" style="display:inline-block;background:#00c2a8;color:#ffffff;text-decoration:none;padding:14px 32px;border-radius:8px;font-weight:600;font-size:1rem;">
        Pick your times
      </a>
    </p>
    <p style="color:#718096;line-height:1.5;font-size:0.8rem;margin:16px 0 0;">
      If the button doesn't work, copy this link into your browser:<br>
      <a href="${esc(pollUrl)}" style="color:#0f4c81;word-break:break-all;">${esc(pollUrl)}</a>
    </p>
    <hr style="border:none;border-top:1px solid #e2e8f0;margin:32px 0 16px;">
    <p style="color:#a0aec0;font-size:0.75rem;line-height:1.5;margin:0;">
      You received this because ${esc(organizerEmail || inviter)} added your address to a meeting poll on MiCal.
    </p>
  </div>
</body></html>`;

  const text = [
    `${inviter} wants you to pick a time for "${pollTitle}".`,
    "",
    `Vote: ${pollUrl}`,
    "",
    "Tip: sign in with your Google or Microsoft calendar and each option will be pre-marked free or busy automatically.",
  ].join("\n");
