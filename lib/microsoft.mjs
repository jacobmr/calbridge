...
export async function listMicrosoftEvents(accessToken, calendarId, startDateTime, endDateTime) {
  const url = new URL(`${MS_GRAPH_BASE}/me/calendars/${encodeURIComponent(calendarId)}/calendarView`);
  url.searchParams.set('startDateTime', startDateTime);
  url.searchParams.set('endDateTime', endDateTime);
  url.searchParams.set('$select', 'id,subject,body,start,end,location,showAs,sensitivity,isAllDay');
...
export async function createMicrosoftEvent(accessToken, calendarId, event) {
  const res = await fetch(`${MS_GRAPH_BASE}/me/calendars/${encodeURIComponent(calendarId)}/events`, {
    method: 'POST',
    headers: {
