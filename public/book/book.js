...
async function loadConfig() {
  ...
  const res = await fetch(`/api/book/public?tenant=${encodeURIComponent(tenant)}&event=${encodeURIComponent(event)}`);
  if (!res.ok) {
...
function parseWorkHours(json) {
  try { return JSON.parse(json); } catch { return { start: '09:00', end: '17:00' }; }
}
...
function renderCalendar() {
  ...
  function build() {
    $('#cal-month-year').textContent = viewDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
    const grid = $('#cal-days');
    grid.innerHTML = '';
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const disabled = date < today || date > maxDate || !isDayAllowed(date, mask);
      if (disabled) {
...
function showConfirmation(data) {
  ...
  $('#confirm-id').textContent = data.booking_id;
  $('#confirm-token').textContent = data.cancel_token;
}
...
async function init() {
  try {
    ...
    if (config.location_mode && config.location_mode !== 'meet') {
      $('#event-location').textContent = `📍 ${config.location_mode}`;
      show($('#event-location'));
    }
