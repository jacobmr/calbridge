...
async function createEventType(req, res) {
  ...
  const {
    slug, name, duration_min, buffer_min, lead_min, horizon_days,
    weekdays_mask, work_hours_json, target_calendar_id, location_mode,
    require_email, pass_required, pass_hash, branding_json, enabled,
  } = body;

  if (!slug || !name || duration_min == null || !work_hours_json || !target_calendar_id) {
    res.statusCode = 400;
    res.setHeader('content-type', 'application/json');
  ...
  await db.execute({
    ...
    args: [
      id, tenant.id, slug, name, duration_min, buffer_min ?? 0, lead_min ?? 0,
      horizon_days ?? 25, weekdays_mask ?? 31, work_hours_json, target_calendar_id,
      location_mode ?? 'meet', require_email ?? 1, pass_required ?? 0, pass_hash ?? null,
      branding_json ?? null, enabled ?? 1,
    ],
  });
...
async function updateEventType(req, res, id) {
  ...
  const allowed = [
    'slug', 'name', 'duration_min', 'buffer_min', 'lead_min',
    'horizon_days', 'weekdays_mask', 'work_hours_json', 'target_calendar_id',
    'location_mode', 'require_email', 'pass_required', 'pass_hash',
    'branding_json', 'enabled',
  ];
